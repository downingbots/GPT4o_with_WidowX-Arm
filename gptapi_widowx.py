...
def send_chat_image(image_path, command, send_prompt=False, bad_response=False):
    # Getting the base64 string
    base64_image = encode_image(image_path)
    if send_prompt:
      text_content = open("prompt.txt",'r').read()
    elif bad_response:
      text_content = "choose the best action for the image: " +
             "FORWARD, BACKWARD, UP, DOWN, LEFT, RIGHT, " +
             "ROTATE_ARM_CLOCKWISE, ROTATE_ARM_COUNTERCLOCKWISE, " +
             "ROTATE_GRIPPER_CLOCKWISE, ROTATE_GRIPPER_COUNTERCLOCKWISE, " +
             "GRIPPER_OPEN, GRIPPER_CLOSE, DONE" 
    else:
      prefix = "What action should the robot arm perform to "
      text_content = prefix + command
...
command = config['language_instruction']
...
while True:
  ...
  display.Image(as_gif(robot_image, True))
  ...
  for i in range(len(robot_actions)):
    if i < 9:
      ...
      print(str(i+1) + " " + robot_actions[i])
  while True:
    gpt_response = send_chat_image(im_file, command, send_prompt, bad_response)
    if gpt_response in robot_actions:
      robot_action = gpt_response
      break
    bad_response = True
  ...
  if robot_action in ["SUCCESS","FAILURE", "DONE"]:
    break
